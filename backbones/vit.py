...
class PatchEmbed(nn.Module):
    def __init__(self, img_size: int = 112, patch_size: int = 28, in_channels: int = 3, embed_dim: int = 768, stride: int = 28):
        ...
        # Unfold expects input of shape (batch_size, in_channels, height, width)
        self.unfold = nn.Unfold(kernel_size=patch_size, stride=stride)
        self.linear = nn.Linear(patch_size[0] * patch_size[1] * in_channels, embed_dim)
    ...
    def forward(self, x):
        ...
        # Shape of unfolded tensor: (batch_size, patch_size*patch_size*in_channels, num_patches)
        unfolded = unfolded.permute(0, 2, 1)  # Shape: (batch_size, num_patches, patch_size^2 * in_channels)
