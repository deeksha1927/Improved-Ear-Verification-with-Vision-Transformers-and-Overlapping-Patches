...
def get_model(name, **kwargs):
    ...
    if name == "r18":
        ...
        stride= kwargs.get("stride", 28)
        from .vit import VisionTransformer
        return VisionTransformer(
            img_size=112, patch_size=patch_size, stride=stride, num_classes=num_features, embed_dim=256, depth=12,
            num_heads=8, drop_path_rate=0.1, norm_layer="ln", mask_ratio=0.0)
        ...
        stride= kwargs.get("stride", 9)
        
        from .vit import VisionTransformer
        return VisionTransformer(
            img_size=112, patch_size=patch_size, stride=stride, num_classes=num_features, embed_dim=512, depth=12,
        ...
        from .vit import VisionTransformer
        return VisionTransformer(
            img_size=112,  patch_size=patch_size, stride=stride, num_classes=num_features, embed_dim=512, depth=24,
        ...
        from .vit import VisionTransformer
        return VisionTransformer(
            img_size=112, patch_size=patch_size, stride=stride, num_classes=num_features, embed_dim=768, depth=24,
            num_heads=8, drop_path_rate=0.05, norm_layer="ln", mask_ratio=0.05, using_checkpoint=True)
