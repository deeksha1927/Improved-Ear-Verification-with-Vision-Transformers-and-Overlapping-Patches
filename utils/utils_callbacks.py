...
class CallBackVerification(object):
    ...
    def ver_test(self, backbone: torch.nn.Module, global_step: int):
        ...
        if avg > self.highest_avg_acc:
            self.highest_avg_acc = avg
            logging.info(f'Ver_Avg: {avg} Best Ver_Avg: {self.highest_avg_acc}')
            return avg
        else:
            logging.info(f'Ver_Avg: {avg} Best Ver_Avg: {self.highest_avg_acc}')
    ...
    def __call__(self, num_update, backbone: torch.nn.Module):
        if self.rank == 0 and num_update > 0:
            backbone.eval()
            best_acc = self.ver_test(backbone, num_update)
            backbone.train()
            return best_acc


class CallBackVerification_test(object):
    ...
    def ver_test(self, backbone: torch.nn.Module, global_step: int):
        results = []
        for i in range(len(self.ver_list)):
            acc1, std1, acc2, std2, xnorm, embeddings_list = verification.test_test(
                self.ver_list[i], backbone, 10, 10)
            logging.info('[%s][%d]XNorm: %f' % (self.ver_name_list[i], global_step, xnorm))
            logging.info('[%s][%d]Accuracy-Flip: %1.5f+-%1.5f' % (self.ver_name_list[i], global_step, acc2, std2))
